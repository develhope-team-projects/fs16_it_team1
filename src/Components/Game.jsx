...
const getArrayStoryFromHero = (hero) => {
    if(hero === HERO.MAGE){
         return storyArray
    }else if(hero === HERO.PALADIN){
        return arrayStoryPaladin
    }else {
        // defaul value 
        return storyArray
    }
...
export function Game() {
    ...
    const [arrayToHero, setArrayToHero] = useState(getArrayStoryFromHero(hero)  )
    const [userStoryArray, setUserStoryArray] = useState([arrayToHero[1]]);
    ...
    const handleUserChoice = (index) => {   //creò la condizione che se l'array è finito gestisci la situazione nell'else
        ...
        //fa comparire un bottone con ricomincia
        if(index === arrayToHero.length-1) {
            setGameOver(true) 
        }
    };
